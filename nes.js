...
function checkRefreshRate() {
    ...
    function check() {
        ...
        if (i > numFrames - 5) {
            ...
            if (ms < 15.5)
                NES.limitFPS = true; // Limit framerate if over 64fps
        }
    }
}
...
$(document).ready(function () {
    ...
    $("#frameskip").change(function () {
        NES.maxFrameSkip = parseInt($("#frameskip")[0].value);
    });
...
class NES {
    ...
    incFrameSkip() {
        if (NES._framesSkipped++ >= NES.maxFrameSkip)
            NES._framesSkipped = 0;
    }
    printDebug() {
        this.print = true;
    }
...
NES.limitFPS = false;
NES.maxFrameSkip = 0; // 0 = No frame skip
NES._framesSkipped = 0;
